...
class PixelCNNLayer_up(nn.Module):
    def __init__(self, nr_resnet, nr_filters, resnet_nonlinearity, rematerialize=False):
...
class PixelCNNLayer_down(nn.Module):
    ...
    def forward(self, u, ul, u_list, ul_list):
        for i in range(self.nr_resnet):
            if self.rematerialize:
                u  = checkpoint(self.u_stream[i], u, u_list.pop())
                ul = checkpoint(self.ul_stream[i], ul, torch.cat((u, ul_list.pop()), 1))
            else:
...
class PixelCNN(nn.Module):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10, 
                    resnet_nonlinearity='concat_elu', input_channels=3, rematerialize=False):
        ...
        self.down_layers = nn.ModuleList([PixelCNNLayer_down(down_nr_resnet[i], nr_filters, 
                                                self.resnet_nonlinearity, rematerialize) for i in range(3)])

        self.up_layers   = nn.ModuleList([PixelCNNLayer_up(nr_resnet, nr_filters, 
                                                self.resnet_nonlinearity, rematerialize) for _ in range(3)])
    ...
    # def forward_checkpointed(self, x, sample=False, mask_init=None, mask_undilated=None, mask_dilated=None):
    #     assert mask_init is None and mask_undilated is None and mask_dilated is None, "PixelCNN++ does not use input masking"
    #     # similar as done in the tf repo. remake init_padding if input height or width change :  
    #     if not sample and (self.init_padding is None or self.init_padding.shape[2:] != x.shape[2:]) :
    #         xs = [int(y) for y in x.size()]
    ...
    #     ###      UP PASS    ###
    #     x = x if sample else torch.cat((x, self.init_padding), 1)
    #     u_list  = [torch.utils.checkpoint(self.u_init, x)]
    #     ul_list = [torch.utils.checkpoint(self.ul_init[0], x) + torch.utils.checkpoint(self.ul_init[1], x)]
    #     for i in range(3):
    #         # resnet block
    #         u_out, ul_out = self.up_layers[i](u_list[-1], ul_list[-1])
    #         u_list  += u_out
